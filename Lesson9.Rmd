...
*CART: classification (con't outcome) and regression tree (discrete outcome)

First, today's vocabulary lesson:

|          Term      |     Definition                                                                |
|--------------------|-------------------------------------------------------------------------------|
| Root node          | Represents the entire pop'n (all the observations), gets divided into 2 more homogeneous subsets     |
...
```
*So now we are talking about majority rather than average
...
people <- people %>% select(Diabetes, Age, BMI, Gender, PhysActive) %>% mutate(fdiab = as.factor(Diabetes))
summary(people)
is.factor(people$fdiab)
...
```{r}
#
diabrf <- rfsrc(fdiab ~ Age + Gender + BMI + PhysActive, data = people, ntree = 1000, tree.err = TRUE)
